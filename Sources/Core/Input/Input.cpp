...
std::vector<int> Input::mKeysPressed {};
std::vector<int> Input::mKeysHolding {};

void Input::Initialize(GLFWwindow* window)
{
	glfwSetKeyCallback(window, [](GLFWwindow* window, int key, int scancode, int action, int mods){
		Input::InternalKeyCallback(window, key, scancode, action, mods);	
	});
}

void Input::InternalKeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	{
		for(std::vector<int>::iterator it = mKeysPressed.begin(); it != mKeysPressed.end(); ++it)
		{
			if(*it == key)
			{
